...
它有一个构造函数，接受一个指向 GlobalDescriptorTable 和一个函数指针 entrypoint
的参数，还有一个析构函数。*/
class Task
{
...
通过传递
GDT，任务能够知道自己的代码和数据所在的段，以便在任务切换时正确地加载这些段。
...
构造函数中通过传递的 entrypoint 函数指针，将任务的 CPU 状态结构中的 eip
字段设置为入口点的地址。

确保正确的权限和访问： GDT 中的描述符定义了各个段的权限和访问规则。
通过使用正确的
GDT，任务可以确保它有权访问其代码和数据，并且这些段的属性符合操作系统的要求。*/

/*TaskManager 类管理一组任务。
它有一个构造函数、一个析构函数以及添加任务 (AddTask) 和调度任务 (Schedule)
的方法。 它维护了一个 Task 指针数组（tasks），最多包含256个任务。
